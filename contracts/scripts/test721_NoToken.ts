...
import { Test721_NoToken__factory} from "../typechain-types";
...
async function main() {
    ...
    const contract_address = "0x1d43437E714e6026e7a061b54ce94315c9E3837F";
    ...
    const doc = Test721_NoToken__factory.connect(contract_address, account_1);
    
    var current_token_id = await doc._tokenIds();
    // console.log("Current token id is : ",current_token_id);

    const terms_url = await doc.termsUrl();
    console.log("Terms URL: ", terms_url);

    const terms_url_with_prefix = await doc.termsUrlWithPrefix("https://");
    ...
    const terms_url_1 = await doc.termsUrl();
    console.log("Terms URL: ", terms_url_1);

    const accept_terms_txn_1 = await doc.acceptTerms(terms_url_1);
    ...
    const accepted_terms_1 = await doc.acceptedTerms(account_1.address);
    console.log("Checking terms accepted by Account_1: ",accepted_terms_1);
    
    const terms_url_2 = await doc.termsUrl();
    ...
    const doc_2 = Test721_NoToken__factory.connect(contract_address, account_2);

    const accepted_terms_2 = await doc_2.acceptedTerms(account_2.address);
    console.log("Checking terms accepted by Account_2: ",accepted_terms_2);

    console.log("Account 2 will be accepting terms for contract now...");

    const accept_terms_txn_2 = await doc_2.acceptTerms(terms_url_1);
    ...
    const accepted_terms_3 = await doc_2.acceptedTerms(account_2.address);
    ...
    // const txn_setTerms = await doc.setTokenTerm("Project Name", "Pooptogether");
