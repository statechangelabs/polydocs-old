...
import { Test1155_Token__factory} from "../typechain-types";
...
async function main() {
    ...
    const contract_address = "0xc0b41E6F8FeB6cf7252072D7850b2d9A2dF43b4B";
    ...
    const doc = Test1155_Token__factory.connect(contract_address, account_1);
    ...
    const mint_txn = await doc.mint(5);
    const mint_receipt = await mint_txn.wait();
    console.log("Minted 5 NFTs to Account 1 hash: ", mint_receipt.blockHash);

    const doc_2 = Test1155_Token__factory.connect(contract_address, account_2);
    ...
    const transfer_txn = await doc_2.safeTransferFrom(account_1.address, account_2.address,current_token_id, ethers2,"");
